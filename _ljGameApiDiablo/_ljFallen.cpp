...
int _ljFallen::mCount = 0;

_ljFallen::_ljFallen(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player)
...
_ljFallen::_ljFallen(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player, TileIndex index)
	: _ljMonster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
void _ljFallen::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == ran)
		{
			_ljGameSound::SoundPlay(L"falld1.wav");
		}
		else
		{
			_ljGameSound::SoundPlay(L"falld2.wav");
		}
		...
		if (true == mGoingToAttack)
		{
			...
			if (nullptr != mDestDibTile->GetActor() || this != mDestDibTile->GetActor())
			{
				if (temp->GetTileIndex() == mDestDibTile->GetTileIndex())
				{
					...
					int ran = _ljGameRandomNum::GetRandomNumber(1, 4);
...
void _ljFallen::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				_ljGameSound::SoundPlay(L"falla1.wav");
			}
			else
			{
				_ljGameSound::SoundPlay(L"falla2.wav");
			}
...
void _ljFallen::Special()
{
	...
	if (false == mIsAttacking)
	{
		...
		if (1 == rand)
		{
			_ljGameSound::SoundPlay(L"falls1.wav");
		}
		else
		{
			_ljGameSound::SoundPlay(L"falls2.wav");
		}
...
void _ljFallen::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void _ljFallen::GetDamage(int damage)
{
	int rand = _ljGameRandomNum::GetRandomNumber(1, 2);

	if (1 == rand)
	{
		_ljGameSound::SoundPlay(L"fallh1.wav");
	}
	else
	{
		_ljGameSound::SoundPlay(L"fallh2.wav");
...
// 가로 인덱스
void _ljFallen::AutoAnimation(const wchar_t* name, int index, float frameSpeed)
{
...
void _ljFallen::AutoAnimation(const wchar_t* name, int index, bool loop, float frameSpeed)
{
...
void _ljFallen::GoingToMove(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void _ljFallen::GoingToAttack(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
