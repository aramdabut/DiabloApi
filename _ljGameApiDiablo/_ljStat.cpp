...
_ljStat::_ljStat()
{
	...
	/*if (nullptr == mBackTexture)
	{
		...
		mBackTexture = _ljGameTextureMgr::Inst().CreateTexture(L"Stats", BACK_TILE_X, BACK_TILE_Y);

		_ljGameTextureMgr::Inst().CreateImage(L"Stats", 1, 1);
...
void _ljStat::SetPlayer(_ljPlayer* player)
{
	mPlayer = player;

	mRed = new _ljGameActor();
	mRed->Scene(mPlayer->Scene());
	mRedRenderer = mRed->CreateRenderer<_ljGameRenderer>(LO_DEBUG);
