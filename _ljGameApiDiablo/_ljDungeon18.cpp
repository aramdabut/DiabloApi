...
void _ljDungeon18::SceneUpdate()
{
	...
	/*
		...
		TextOutW(_ljGameTextureMgr::BackDC(), 0, 80, Arr, lstrlen(Arr));
	*/
...
void _ljDungeon18::Init()
{
	mBGM = _ljGameSound::SoundPlay(L"dlvlc.mp3");
}

void _ljDungeon18::MakeMap()
{
	Make3RoomsY();
...
void _ljDungeon18::SetPlayer()
{
	_ljColMap* colMap = mMap1->ColMap();
	colMap->SetPlayer(mPlayer);
}

void _ljDungeon18::SetMonster()
{
	SpawnMonster();
}

void _ljDungeon18::SpawnMonster()
{
	int size = 0;
	int ran = 0;
	int x = 0;
	...
	for (size_t i = 0; i < roomSize; ++i)
	{
		...
		{
			ran = _ljGameRandomNum::GetRandomNumber(1, 6);
...
void _ljDungeon18::ResetPosFrom15()
{
...
void _ljDungeon18::ResetPosFrom2()
{

	// Dungeon18에서의 플레이어 정보를 받아온다.
	_ljPlayer* tempP = mDungeon2->GetPlayer();
