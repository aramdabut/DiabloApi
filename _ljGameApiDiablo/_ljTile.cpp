...
int _ljTile::mUpstairCount = 0;
int _ljTile::mDownstairCount = 0;

_ljTile::_ljTile()
{
}

_ljTile::_ljTile(const _ljTile& other)
	: mMap(other.mMap)
	, mX(other.mX)
	, mY(other.mY)
...
_ljTile::_ljTile(_ljRandomMap* map, int x, int y, LEAGUE_ORDER order)
	: mMap(map)
	, mX(x)
	, mY(y)
	, mRenderOrder(order)
	...
	, mIsDebug(true)
{
	...
	/*mTileRenderer = CreateRenderer<_ljGameRenderer>(LO_TILE);
	...
	mWallRenderer = CreateRenderer<_ljGameRenderer>(order);
	...
	mFogRenderer = CreateRenderer<_ljGameRenderer>(order);
...
void _ljTile::DebugRender()
{
	wchar_t Arr[256] = { 0, };
	swprintf_s(Arr, L"%d, %d", mX, mY);
	TextOutW(_ljGameTextureMgr::BackDC(), CalPos().IntX() + 44, CalPos().IntY() + 8 - 64, Arr, lstrlen(Arr));
}

void _ljTile::DebugOutputTile()
{
	wchar_t Arr[200] = { 0, };
...
void _ljTile::Update()
{
	if (mObject != nullptr)
	{
		if (CHT_LDOOR == mObject->GetChestType())
		{
			int a = 0;
		}
	}
	...
	if (true == _ljGameInput::Down(L"OUTPUT"))
	{
		DebugOutputTile();
	}
...
_ljObject* _ljTile::PutBox(int index)
{
	if (nullptr == mObject)
	{
		// mTileType = TT_WALL;
		CHEST_TYPE type = (CHEST_TYPE)index;

		if (type == CHT_RDOOR)
		{
			mObject = new _ljObject(index, Pos(), { mX * 2 + 1 , mY * 2 });
		}
		else if (type == CHT_LDOOR)
		{
			mObject = new _ljObject(index, Pos(), { mX * 2 , mY * 2 + 1 });
		}
		else
		{
			mObject = new _ljObject(index, Pos(), { mX * 2 , mY * 2 });
		}
	}
	else
	{
