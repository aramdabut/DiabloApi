...
int _ljSkeleton::mCount = 0;


_ljSkeleton::_ljSkeleton(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player)
...
_ljSkeleton::_ljSkeleton(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player, TileIndex index)
	: _ljMonster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
_ljSkeleton::~_ljSkeleton()
{
}

void _ljSkeleton::DebugRender()
{
	wchar_t Arr[256] = { 0, };
...
void _ljSkeleton::CheckDirChange()
{
...
void _ljSkeleton::Special()
{
}

void _ljSkeleton::Rest()
{
	mIsAttacking = false;
...
void _ljSkeleton::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void _ljSkeleton::GetDamage(int damage)
{
...
// 가로 인덱스
void _ljSkeleton::AutoAnimation(const wchar_t* name, int index, float frameSpeed)
{
...
void _ljSkeleton::GoingToMove(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void _ljSkeleton::GoingToAttack(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
