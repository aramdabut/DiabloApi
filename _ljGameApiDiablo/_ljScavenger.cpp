...
int _ljScavenger::mCount = 0;


_ljScavenger::_ljScavenger(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player)
...
_ljScavenger::_ljScavenger(_ljColMap* colmap, _ljRandomMap* randomMap, _ljPlayer* player, TileIndex index)
	: _ljMonster(colmap, randomMap)
	, mPlayer(player)
	, mIsPlayerAround(true)
	, mIsExpDone(false)
...
_ljScavenger::~_ljScavenger()
{
}

void _ljScavenger::DebugRender()
{
	wchar_t Arr[256] = { 0, };
...
void _ljScavenger::StateUpdate()
{
	...
	if (0 >= mCurHP && false == mIsDead)
	{
		...
		if (1 == rand)
		{
			_ljGameSound::SoundPlay(L"scavd1.wav");
		}
		else
		{
			_ljGameSound::SoundPlay(L"scavd2.wav");
		}
...
void _ljScavenger::CheckDirChange()
{
...
void _ljScavenger::Attack()
{
	...
	else
	{
			...
			false == mAttacked)
		{
			...
			if (1 == rand)
			{
				_ljGameSound::SoundPlay(L"scava1.wav");
			}
			else
			{
				_ljGameSound::SoundPlay(L"scava2.wav");
			}
...
void _ljScavenger::Special()
{
}

void _ljScavenger::Rest()
{
	mIsAttacking = false;
...
void _ljScavenger::SetCurrentState(MONSTER_STATE state, bool noneDir /*= false*/)
{
	mState = state;
...
void _ljScavenger::GetDamage(int damage)
{
	int rand = _ljGameRandomNum::GetRandomNumber(1, 2);

	if (1 == rand)
	{
		_ljGameSound::SoundPlay(L"scavh1.wav");
	}
	else
	{
		_ljGameSound::SoundPlay(L"scavh2.wav");
	}
...
void _ljScavenger::SetAnimation()
{
	// SetAnimation
	mAllDirName[T] = L"T";
	mAllDirName[LT] = L"LT";
...
// 가로 인덱스
void _ljScavenger::AutoAnimation(const wchar_t* name, int index, float frameSpeed)
{
...
void _ljScavenger::AutoAnimation(const wchar_t* name, int index, bool loop, float frameSpeed)
{
...
void _ljScavenger::GoingToMove(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
...
// 찍은 곳이 몬스터가 있는곳.
// 마지막 타일을 pop_back한다.
void _ljScavenger::GoingToAttack(_ljVector mousePos)
{
	mPathList.clear();
	// 길찾기
	mPathList = mColMap->PathFindM(CalPos(), mousePos);
