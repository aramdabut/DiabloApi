...
// std::vector<_ljRandomMap*> _ljRandomMap::mAllMap;

int _ljRandomMap::mLevel = 0;

// 지도를 만들때 반드시 층을 적어주어야 한다.
_ljRandomMap::_ljRandomMap()
{
	mLevel = -1;
...
// 지도를 만들때 반드시 층을 적어주어야 한다.
_ljRandomMap::_ljRandomMap(_ljGameScene* scene)
{
	mHasUpstair = false;
	mHasDownstair = false;

	mLastRoom = new _ljRoom();

	mAllTile = new _ljTile * *[MAPCOUNT_Y];
	mAllCopyTile = new _ljTile * *[MAPCOUNT_Y];


	mColliderMap = new _ljColMap(this);

	//// 타일 생성
	for (int i = 0; i < MAPCOUNT_Y; i++)
	{
		mAllTile[i] = new _ljTile * [MAPCOUNT_X]();

		for (int j = 0; j < MAPCOUNT_X; j++)
		{
			mAllTile[i][j] = new _ljTile(this, j, i, LO_UPTILE);
		}
	}

	for (int i = 0; i < MAPCOUNT_Y; i++)
	{
		mAllCopyTile[i] = new _ljTile * [MAPCOUNT_X]();

		for (int j = 0; j < MAPCOUNT_X; j++)
		{
			mAllCopyTile[i][j] = new _ljTile(this, j, i, LO_DOWNTILE);
		}
	}

	mBackTile = new _ljGameActor();

	// 랜덤맵 시드 설정
	_ljGameRandomNum::SetSeed(_ljGameRandomNum::TimeRandomUInt());
	...
	{
		_ljGameRenderer* temp = mBackTile->CreateRenderer<_ljGameRenderer>(LO_TILE);
		temp->SetRenderStyle(RS_BIT);
		temp->SubSize(_ljVector(BACK_TILE_X, BACK_TILE_Y));
		temp->SubPos(_ljVector(0.f - 18.f, BACK_TILE_Y / 2.f - 166));
		...
		mBackTexture = _ljGameTextureMgr::Inst().CreateTexture(tName.c_str(), BACK_TILE_X, BACK_TILE_Y);

		_ljGameTextureMgr::Inst().CreateImage(tName.c_str(), 1, 1);

		temp->Image(tName.c_str());
	}
}

_ljRandomMap::_ljRandomMap(const _ljRandomMap& other)
{
	mAllTile = new _ljTile * *[MAPCOUNT_Y];
...
void _ljRandomMap::Update()
{

}

void _ljRandomMap::SetPlayer(_ljPlayer* player)
{
	mPlayer = player;
...
_ljPlayer* _ljRandomMap::GetPlayer() const
{
	return mPlayer;
}

_ljTile* _ljRandomMap::GetTile(int _X, int _Y)
{
...
_ljTile* _ljRandomMap::GetTile(_ljVector pos)
{
...
void _ljRandomMap::Make3RoomsY(int level)
{

}

bool _ljRandomMap::MakeARoom(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType)
{
	int X = basePos.IntX();
	int Y = basePos.IntY();

	_ljVector TravellerBase = basePos + _ljVector::DOWNRIGHT;
	...
	for (; y < size.IntY(); y++)
	{
		_ljVector TravellerPos = TravellerBase;
		...
		for (; x < size.X; x++)
		{
			...
			TravellerPos += _ljVector::RIGHT;
		}

		TravellerBase = TravellerBase + _ljVector::DOWN;
	}
	...
	_ljVector tempSize = { (float)x, (float)y };
	...
	///////////////////////////////////////////////////////////////////////////////////////////////// 문
	// 문 생성하기 (뚫림 1:2 문)
	int doorRandom = _ljGameRandomNum::GetRandomNumber(1, 10);
...
void _ljRandomMap::MakeTopEleven(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize)
{
	...
	//_ljVector TravellerBase = basePos;
	...
	//Room* tempRoom = new Room(basePos, size - _ljVector{ 2, 2 });
	//mMap->AddRoom(tempRoom);
}

void _ljRandomMap::MakeLeftEleven(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize)
{
}

void _ljRandomMap::MakeArchY(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize)
{
	_ljVector leftTop = basePos;
	_ljVector leftDown = basePos + _ljVector(0.f, size.Y + 1);
	_ljVector rightTop = basePos + _ljVector(size.X + 1, 0.f);
	_ljVector rightDown = basePos + size + 1;
...
void _ljRandomMap::MakeArchX(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize)
{
}

void _ljRandomMap::MakeRandomRooms(_ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType, int left, int bottom, int right, int top)
{
	mLeftLimit = left;
	mBottomLimit = bottom;
	mRightLimit = right;
	...
	// 방크기 랜덤 생성
	int sizeX = _ljGameRandomNum::GetRandomNumber(2, 8);
	int sizeY = _ljGameRandomNum::GetRandomNumber(2, 8);
	...
	_ljVector pos = { posX, posY };
	_ljVector size = { sizeX, sizeY };
	...
	/*if (false == mHasUpstair)
	{
		if (sizeX >= 4 && sizeY >= 4)
		{
			_ljVector stairPos = CheckNorthWall(pos, size);

			if (_ljVector{ -1, -1 } != stairPos)
			{
				stairPos += { 0, -1 };
	...
	else if (false == mHasDownstair)
	{
		if (sizeX >= 4 && sizeY >= 5)
		{
			...
			if (_ljVector{ -1, -1 } != _ljVector{ posX, posY })
			{
				MakeDownstair(pos);
			}
		}
...
void _ljRandomMap::MakeRandomBRooms(_ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType, int left, int bottom, int right, int top)
{
	mLeftLimit = left;
	mBottomLimit = bottom;
	mRightLimit = right;
	...
	// 방크기 랜덤 생성
	int sizeX = _ljGameRandomNum::GetRandomNumber(3, 7);
	int sizeY = _ljGameRandomNum::GetRandomNumber(3, 7);
...
bool _ljRandomMap::MakeButchersPreRoom(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType)
{
	if (1 > basePos.IntX())
	{
		int X = 1;
	...
	MakeButchersRealRoom({ basePos.X + 2, basePos.Y + 2 }, newSize - _ljVector({ 4, 4 }), prevPos, prevSize, RT_MAX);

	return true;
}

bool _ljRandomMap::MakeButchersRealRoom(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType)
{
	int X = basePos.IntX();
	int Y = basePos.IntY();
	...
	////////////////////// 이제 실제 방을 만들자
	TravellerBase = basePos + _ljVector::DOWNRIGHT;
	for (; y < size.IntY(); y++)
	{
		_ljVector TravellerPos = TravellerBase;
	...
	MakeBFloor(basePos + _ljVector::DOWNRIGHT, { 2, 2 });
	...
	SetDoorAt(_ljVector{ basePos.X + 3, basePos.Y + 2 }, WT_BDOOR);
	...
	for (size_t i = 7; i < mAllRoom.size(); ++i)
	{
		...
		else if (false == mHasDownstair)
		{
			if (sizeX >= 4 && sizeY >= 5)
			{
				...
				if (_ljVector{ -1, -1 } != _ljVector{ pos.X, pos.Y })
				{
					MakeDownstair(pos);
				}
			}
...
void _ljRandomMap::SetDownstair()
{
}

void _ljRandomMap::MakeUpstair(_ljVector& basePos)
{
	if (basePos.Y <= 0)
	{
		return;
...
void _ljRandomMap::MakeDownstair(_ljVector& basePos)
{
	_ljVector pos = basePos + _ljVector::RIGHT + _ljVector::DOWN + _ljVector::RIGHT + _ljVector::DOWN;
	...
	for (int y = 0; y < 2; y++)
	{
		...
		pos = basePos + _ljVector::RIGHT + _ljVector::DOWN + _ljVector::RIGHT + _ljVector::DOWN + _ljVector::DOWN;
	}

	pos = basePos + _ljVector::RIGHT + _ljVector::DOWN + _ljVector::RIGHT + _ljVector::DOWN;
...
bool _ljRandomMap::MakeADoor(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType)
{
	// 큰 순으로 정렬
	...
	int ranY = _ljGameRandomNum::GetRandomNumber(1, (int)randomSizeY);
	int ranX = _ljGameRandomNum::GetRandomNumber(1, (int)randomSizeX);
	...
	case RT_LEFT:
		...
		SetDoorAt(_ljVector{ bigPos.X, bigPos.Y + ranY }, WT_LEFTDOOR);
		break;
	case RT_BOTTOM:
		...
		SetDoorAt(_ljVector{ bigPos.X + ranX, bigPos.Y }, WT_BOTTOMDOOR);
		break;
	case RT_RIGHT:
		if (TT_STAIR == mAllTile[bigPos.IntY() + ranY][bigPos.IntX()]->TileType())
		{
		...
		SetDoorAt(_ljVector{ bigPos.X, bigPos.Y + ranY }, WT_RIGHTDOOR);
		break;
	case RT_TOP:
		if (TT_STAIR == mAllTile[bigPos.IntY()][bigPos.IntX() + ranX]->TileType())
		{
		...
		SetDoorAt(_ljVector{ bigPos.X + ranX, bigPos.Y }, WT_TOPDOOR);
		break;
	case RT_MAX:
		break;
	default:
...
bool _ljRandomMap::MakeAConnection(_ljVector basePos, _ljVector size, _ljVector prevPos, _ljVector prevSize, ROOM_TYPE roomType)
{
	// 큰 순으로 정렬
	float arrX[4] = { basePos.X, basePos.X + size.X, prevPos.X, prevPos.X + prevSize.X };
	float arrY[4] = { basePos.Y, basePos.Y + size.Y, prevPos.Y, prevPos.Y + prevSize.Y };
	...
	case RT_LEFT:
		...
		MakeWideFloor(bigPos + _ljVector::DOWN, { 1.f, arrY[2] - arrY[1] - 1 });
	...
	case RT_BOTTOM:
		...
		MakeWideFloor(bigPos + _ljVector::RIGHT, { arrX[2] - arrX[1] - 1, 1.f });
...
void _ljRandomMap::MakeFloor(_ljVector basePos, _ljVector size)
{
	_ljVector TravellerBase = basePos;
	_ljVector TravellerPos;
	for (int y = 0; y < size.Y; y++)
	{
	...
	TravellerPos = TravellerPos + _ljVector::DOWN + _ljVector::LEFT;
...
void _ljRandomMap::MakeBFloor(_ljVector basePos, _ljVector size)
{
	_ljVector TravellerBase = basePos;
	_ljVector TravellerPos;
	for (int y = 0; y < size.Y; y++)
...
void _ljRandomMap::MakeWideFloor(_ljVector basePos, _ljVector size)
{
	_ljVector newSize = size;
...
void _ljRandomMap::UpgradeRoomCorner(_ljVector basePos, _ljVector size)
{
	int x = basePos.IntX();
	int y = basePos.IntY();
...
void _ljRandomMap::UpgradeAdjCorner(int x, int y, WALL_TYPE type)
{
	if (x == 39 && y == 1)
	{
		int a = 0;
...
void _ljRandomMap::UpgradeAllAdjCorner()
{
...
void _ljRandomMap::SetFloorAt(int x, int y)
{
...
void _ljRandomMap::SetFloorAt(_ljVector pos)
{
...
void _ljRandomMap::SetWallAt(int x, int y, WALL_TYPE type)
{
	if (x < 0 || x > MAPSIZE_X - 1 ||
		y < 0 || y > MAPSIZE_Y - 1)
	{
...
void _ljRandomMap::SetWallAt(_ljVector pos, WALL_TYPE type)
{
	SetWallAt(pos.IntX(), pos.IntY(), type);
}

void _ljRandomMap::SetDoorAt(_ljVector pos, WALL_TYPE doorType)
{
...
void _ljRandomMap::SetCornerAt(int x, int y, WALL_TYPE type)
{
	if (x < 0 || x > MAPSIZE_X - 1 ||
		y < 0 || y > MAPSIZE_Y - 1)
	{
...
void _ljRandomMap::SetCornerAt(_ljVector pos, WALL_TYPE type)
{
	SetCornerAt(pos.IntX(), pos.IntY(), type);
}

// 덮어쓰기 하지 않음
void _ljRandomMap::SetNormalCornerAt(_ljVector pos, WALL_TYPE type)
{
	if (pos.IntX() < 0 || pos.IntX() > MAPSIZE_X - 1 ||
		pos.IntY() < 0 || pos.IntY() > MAPSIZE_Y - 1)
	{
...
void _ljRandomMap::SetColumnAt(int x, int y)
{
	if (x < 0 || x > MAPSIZE_X - 1 ||
		y < 0 || y > MAPSIZE_Y - 1)
	{
...
void _ljRandomMap::SetWallAroundAt(int x, int y)
{
	// 1. x, y 타일 주변 8방향을 모두 벽으로 만든다.
	for (int i = 0; i < DT_MAX; i++)
	{
		_ljVector newPos = _ljVector(x, y) + _ljVector::ARRDIR[i];
		...
		// 그 타일에다가
		// _ljTile temp = mAllTile[newPos.IntY()][newPos.IntX()];
...
void _ljRandomMap::SetBWallAroundAt(int x, int y)
{
	// 1. x, y 타일 주변 8방향을 모두 벽으로 만든다.
	for (int i = 0; i < DT_MAX; i++)
	{
...
void _ljRandomMap::SetWallAroundAt(_ljVector pos)
{
}



bool _ljRandomMap::CheckIsRoom(_ljVector& roomPos, _ljVector& roomSize)
{
	_ljVector size = roomSize;

	_ljVector basePos = roomPos;
	...
	for (int i = 0; i < mAllRoom.size(); ++i)
	{
		...
		{
			_ljVector travellerPos = basePos;
			...
			basePos = basePos + _ljVector::DOWN;
		}
	}
...
_ljVector _ljRandomMap::CheckNorthWall(_ljVector& roomPos, _ljVector& roomSize)
{
	int count = 0;
	int limit = 3;
	...
	_ljVector stairPosition = { roomPos.IntX() + x - 3 - random, roomPos.IntY() };

	return stairPosition;
}

// 인자로 넣은 타일이 주변에 있는지 확인
bool _ljRandomMap::CheckIsTile(int x, int y, WALL_TYPE type)
{
...
// 인자로 넣은 타일이 주변에 있는지 확인
bool _ljRandomMap::CheckIsTile(int x, int y, TILE_TYPE type)
{
...
bool _ljRandomMap::IsFloor(int x, int y)
{
	if (x < 0 || x >= MAPSIZE_X ||
		y < 0 || y >= MAPSIZE_Y)
	{
...
bool _ljRandomMap::IsFloor(_ljVector pos)
{
...
bool _ljRandomMap::IsWall(int x, int y)
{
	if (x < 0 || x >= MAPSIZE_X ||
		y < 0 || y >= MAPSIZE_Y)
	{
...
bool _ljRandomMap::IsWall(_ljVector pos)
{
	if (pos.IntX() < 0 || pos.IntX() >= MAPSIZE_X ||
		pos.IntX() < 0 || pos.IntY() >= MAPSIZE_Y)
	{
...
bool _ljRandomMap::IsRoom(int x, int y)
{
	if (x < 0 || x >= MAPSIZE_X ||
		y < 0 || y >= MAPSIZE_Y)
	{
...
bool _ljRandomMap::IsRoom(_ljVector pos)
{
	if (pos.IntX() < 0 || pos.IntX() >= MAPSIZE_X ||
...
bool _ljRandomMap::IsUpstair(_ljVector pos)
{
	if (pos.IntX() < 0 || pos.IntX() >= MAPSIZE_X ||
		pos.IntX() < 0 || pos.IntY() >= MAPSIZE_Y)
	{
...
bool _ljRandomMap::IsDownstair(_ljVector pos)
{
	if (pos.IntX() < 0 || pos.IntX() >= MAPSIZE_X ||
		pos.IntX() < 0 || pos.IntY() >= MAPSIZE_Y)
	{
...
_ljRandomMap& _ljRandomMap::CopyMap(_ljRandomMap* map)
{
	for (int i = 0; i < MAPCOUNT_Y; i++)
	{
		for (int j = 0; j < MAPCOUNT_X; j++)
...
WALL_TYPE _ljRandomMap::StyleFloor()
{
	int random = _ljGameRandomNum::GetRandomNumber(1, 9);
...
WALL_TYPE _ljRandomMap::StyleLeftWall()
{
	int random = _ljGameRandomNum::GetRandomNumber(1, 8);
...
WALL_TYPE _ljRandomMap::StyleTopWall()
{
	int random = _ljGameRandomNum::GetRandomNumber(1, 10);
...
void _ljRandomMap::InstallObjects()
{
	for (int y = 0; y < MAPCOUNT_Y; y++)
	{
		for (int x = 0; x < MAPCOUNT_X; x++)
		{
			if (WT_BOTTOMDOOR == mAllTile[y][x]->WallType() || WT_TOPDOOR == mAllTile[y][x]->WallType())
			{
				_ljObject* ob = mAllTile[y][x]->PutBox(CHT_RDOOR);

				_ljColTile* tile1 = mColliderMap->FindTile(TileIndex({ x * 2 + 1, y * 2 }));

				tile1->IsMovable(false);
				tile1->SetObject(ob);
			}
			else if (WT_RIGHTDOOR == mAllTile[y][x]->WallType() || WT_LEFTDOOR == mAllTile[y][x]->WallType() || WT_BDOOR == mAllTile[y][x]->WallType())
			{
				_ljObject* ob = mAllTile[y][x]->PutBox(CHT_LDOOR);

				_ljColTile* tile1 = mColliderMap->FindTile(TileIndex({ x * 2, y * 2 + 1 }));
			...
			else if (WT_FLOOR1_B == mAllTile[y][x]->WallType() ||
			...
			{
				int rand = _ljGameRandomNum::GetRandomNumber(1, 10);

				if (1 == rand)
				{
					int random = _ljGameRandomNum::GetRandomNumber(1, 5);

					switch (random)
					{
					case 1:
						PlaceBox((int)x, (int)y, CHT_SILVER);
						break;
					case 2:
						PlaceBox((int)x, (int)y, CHT_BIG);
						break;
					case 3:
						PlaceBox((int)x, (int)y, CHT_BARREL);
						break;
					case 4:
						PlaceBox((int)x, (int)y, CHT_SARCO);
						break;
					case 5:
						PlaceBox((int)x, (int)y, CHT_SMALL);
						break;
					default:
						break;
					}
	...
	// PlaceBox((int)mStartPos1.X / 2, (int)mStartPos1.Y / 2 + 4, CHT_SARCO);
}


void _ljRandomMap::SetAllShadow()
{
	for (size_t y = 0; y < MAPCOUNT_Y; y++)
	{
		for (size_t x = 0; x < MAPCOUNT_X; x++)
...
void _ljRandomMap::PlaceBox(int x, int y, CHEST_TYPE index)
{
	// 주변에 벽이 없고 박스가 없으면
	if (false == CheckIsTile(x, y, TT_WALL) &&
		false == CheckIsTile(x, y, TT_STAIR))
	{
		_ljObject* ob = mAllTile[y][x]->PutBox(index);

		if (index == CHT_SARCO)
		{
			_ljColTile* tile1 = mColliderMap->FindTile(TileIndex({ x * 2, y * 2 }));
			_ljColTile* tile2 = mColliderMap->FindTile(TileIndex({ x * 2, y * 2 + 1 }));
...
void _ljRandomMap::SetIronWallX(_ljVector basePos, int size)
{
...
void _ljRandomMap::SetIronWallY(_ljVector basePos, int size)
{
	for (size_t i = 0; i < size; ++i)
	{
		if (basePos.IntY() + i >= 40 || basePos.IntX() + i >= 40)
...
bool _ljRandomMap::UpgradeIronWall()
{
	for (size_t i = 0; i < mAllRoom.size(); ++i)
	{
		int random = _ljGameRandomNum::GetRandomNumber(1, 4);
...
bool _ljRandomMap::MakeIronWall(_ljVector basePos, _ljVector size, ROOM_TYPE type)
{
...
void _ljRandomMap::UpgradeWallStyle()
{
	WALL_TYPE temp;
	...
	for (size_t y = 0; y < MAPCOUNT_Y; y++)
	{
		for (size_t x = 0; x < MAPCOUNT_X; x++)
		{
			...
			if (TT_FLOOR == mAllTile[y][x]->TileType() ||
				WT_FLOOR == mAllTile[y][x]->WallType())
			{
				int random = _ljGameRandomNum::GetRandomNumber(1, 20);
...
void _ljRandomMap::SetColliderMap()
{
	for (size_t y = 0; y < MAPCOUNT_Y; ++y)
	{
		for (size_t x = 0; x < MAPCOUNT_X; ++x)
...
// 벽이 왼쪽이라는 뜻
void _ljRandomMap::AddLeft(int x, int y)
{
...
void _ljRandomMap::AddRight(int x, int y)
{
	mColliderMap->AddTile(x, y);
	mColliderMap->AddTile(x + 1, y);
	mColliderMap->AddTile(x, y + 1);
...
void _ljRandomMap::AddAll(int x, int y)
{
	mColliderMap->AddTile(x, y);
	mColliderMap->AddTile(x + 1, y);
	mColliderMap->AddTile(x, y + 1);
...
void _ljRandomMap::AddLT(int x, int y)
{
	mColliderMap->AddTile(x, y);
	mColliderMap->AddTile(x + 1, y);
	mColliderMap->AddTile(x, y + 1);
...
void _ljRandomMap::AddT(int x, int y)
{
	mColliderMap->AddTile(x, y);
	mColliderMap->AddTile(x + 1, y);
	mColliderMap->AddTile(x, y + 1);
...
void _ljRandomMap::AddNone(int x, int y)
{
	mColliderMap->AddTile(x, y);
	mColliderMap->AddTile(x + 1, y);
	mColliderMap->AddTile(x, y + 1);
...
_ljVector _ljRandomMap::SubToDib(int x, int y)
{
	int tempX = (int)(x / 2);
	int tempY = (int)(y / 2);

	return _ljVector(tempX, tempY);
}

void _ljRandomMap::AddSubTile(_ljColTile* subTile)
{
	TileIndex tempIndex = subTile->GetTileIndex();

	_ljVector tempDib = SubToDib(tempIndex.X, tempIndex.Y);

	mAllTile[tempDib.IntX()][tempDib.IntY()]->mSubTiles.push_back(subTile);
}

void _ljRandomMap::RenderTile()
{

}

void _ljRandomMap::SetBRoom()
{
	for (size_t i = 7; i < mAllRoom.size(); ++i)
	{
		int sizeX = mAllRoom[i]->Size.IntX();
...
void _ljRandomMap::SetStartRoom()
{
	for (size_t i = 0; i < mAllRoom.size(); i++)
	{
...
void _ljRandomMap::SetImageIndex()
{
	int index = 0;
	...
	for (size_t y = 0; y < MAPCOUNT_Y; ++y)
	{
		for (size_t x = 0; x < MAPCOUNT_X; ++x)
		{
			...
			_ljGameImage* image = _ljGameTextureMgr::Inst().FindImage(L"ChurchDungeonTile.bmp");
			...
			_ljVector pos = { newX + BACK_TILE_X / 2, newY };
			_ljVector size = { TILE_WIDTH, TILE_HEIGHT };

			int transColor = _ljGameGraphics::ColorToInt(255, 255, 255, 0);
...
void _ljRandomMap::SetCopyImageIndex()
{
	for (size_t y = 0; y < MAPCOUNT_Y; ++y)
	{
		for (size_t x = 0; x < MAPCOUNT_X; ++x)
